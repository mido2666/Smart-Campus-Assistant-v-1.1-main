...
import { Bell, FileText, GraduationCap, AlertTriangle, MessageSquare } from 'lucide-react';

export type FilterType = 'all' | 'assignment' | 'grade' | 'system' | 'message';
...
export default function FilterBar({ activeFilter, onFilterChange, unreadCount }: FilterBarProps) {
  const getColor = (key: FilterType, isActive: boolean) => {
    ...
    switch (key) {
      ...
      case 'grade': return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 ring-1 ring-green-500/20';
