...
// Filter options
export const dayOptions = ['Days', 'Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday'];
export const statusOptions = ['Status', 'Upcoming', 'Ongoing', 'Completed'];
...
export const loadScheduleFilters = (): FilterOptions => {
  try {
    ...
    return saved ? JSON.parse(saved) : { day: 'Days', status: 'Status' };
  } catch (error) {
    console.error('Error loading schedule filters:', error);
    return { day: 'Days', status: 'Status' };
  }
};